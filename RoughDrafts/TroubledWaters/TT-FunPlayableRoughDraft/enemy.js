class Enemy {
  ...
  static spawnEnemy(pos) {
    ...
    if(posMirror.x < width/2 && posMirror.y < height/2) {
      ...
      if(random() < 0.5) {
        return new Enemy(random(0, width/2), 0,
            createVector(random(velMax), random(velMax)));
      }
      // Left
      else {
        return new Enemy(0, random(0, height/2),
            createVector(random(velMax), random(velMax)));
      }
    }
    ...
    if(posMirror.x > width/2 && posMirror.y < height/2) {
       // Top
      if(random() < 0.5) {
        return new Enemy(random(width/2, width), 0,
            createVector(random(-velMax, velMax), random(velMax)));
      }
      // Right
      else {
        return new Enemy(width, random(0, height/2),
